...
- 做國中會考數學 PDF／歷屆試題轉 LaTeX CSV、套用 `template.csv`、或抽取 LaTeX figure assets 時，讀 [references/math-pdf-latex-csv/README.md](references/math-pdf-latex-csv/README.md)。
...
6. 若輸出 LaTeX 題庫或圖片資源，需同步輸出可重跑腳本、索引檔與驗證摘要，並區分自動裁切與人工校對資源。
7. 交付時說清楚本次執行了哪些步驟、哪些欄位是新增或改寫。
...
- 若產出圖片 assets，需提供 `index.csv` 或等價索引，記錄來源 PDF／頁碼／裁切座標／產出方法。
